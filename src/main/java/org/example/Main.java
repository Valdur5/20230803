...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    // Classes in the details package are more concrete implementations for example how to read
    // exactly. The given file format. There should be NO dependency to the core package from the details
    // package.
    Main main = new Main();
  ...
  private void calculateAndPrintAllPathsWithLatencyLimit(
      String startName, String endName, int latencyLimit) {
    this.printLine(
        () ->
            this.traverseService.findAllPathsWithLessThanLatency(startName, endName, latencyLimit));
  }

  private void initGraphDialog() {
    System.out.println(
        "In case you want to load another file than the default [" + DEFAULT_GRAPH_DATA + "].");
    System.out.println(
        "Please enter the absolute path and file name of the file with the graph definition.");
  ...
  private void calculateAndPrintNumberOfTracesWithHops(
      String startName, String endName, int maxHops, boolean onlyExactHops) {
    this.printLine(
        () ->
            this.traverseService.findNumberOfPossibleTraces(
                startName, endName, maxHops, onlyExactHops));
  }
